...
{
    /// Fires several pellets in a spread cone per shot instead of one hitscan
    /// ray - shorter effective range, heavier close-up damage than the pistol.
        ...
        void Fire()
        {
            ...
            for (int i = 0; i < pelletCount; i++)
            {
                Vector3 dir = Quaternion.Euler(
                    ...
                    Random.Range(-spreadDegrees, spreadDegrees),
                    0f) * playerCamera.transform.forward;

                if (Physics.Raycast(playerCamera.transform.position, dir, out var hit, data.range, hitMask, QueryTriggerInteraction.Ignore))
                {
